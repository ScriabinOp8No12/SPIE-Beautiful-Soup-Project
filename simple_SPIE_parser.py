...
def parse_exhibitors_html(html_file, exhibition_type):
    ...
    Args:
        ...
        exhibition_type: 'WEST' or 'BIOS' for filename
    
    Returns:
        DataFrame with company information
    """
    print(f"\n{'='*60}")
    print(f"Processing {exhibition_type} exhibitors from {html_file}...")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    # Get timestamp once for both files
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    
    # Process WEST first
    west_html = 'WEST_11_24_2025.html'
    if os.path.exists(west_html):
        west_df = parse_exhibitors_html(west_html, 'WEST')
        west_output_file = os.path.join(output_folder, f'SPIE_WEST_{timestamp}.csv')
        west_df.to_csv(west_output_file, index=False, encoding='utf-8')
        print(f"\n✓ WEST scraping complete!")
        print(f"  Total companies scraped: {len(west_df)}")
        print(f"  Data saved to: {west_output_file}")
    else:
        print(f"\n⚠ Warning: {west_html} not found. Skipping WEST parsing.")
    
    # Process BIOS second
    bios_html = 'BIOS_11_24_2025.html'
    if os.path.exists(bios_html):
        bios_df = parse_exhibitors_html(bios_html, 'BIOS')
        bios_output_file = os.path.join(output_folder, f'SPIE_BIOS_{timestamp}.csv')
        bios_df.to_csv(bios_output_file, index=False, encoding='utf-8')
        print(f"\n✓ BIOS scraping complete!")
        print(f"  Total companies scraped: {len(bios_df)}")
        print(f"  Data saved to: {bios_output_file}")
    else:
        print(f"\n⚠ Warning: {bios_html} not found. Skipping BIOS parsing.")
